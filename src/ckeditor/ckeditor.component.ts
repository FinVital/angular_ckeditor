...
export class CKEditorComponent implements AfterViewInit, OnDestroy, ControlValueAccessor {
	 ...
	 */
	@Input() editorUrl = 'https://cdn.ckeditor.com/4.20.1/standard-all/ckeditor.js';
	...
	ngAfterViewInit(): void {
		  getEditorNamespace( this.editorUrl, (namespace: any) => {
	...
	private createEditor(): void {
		...
		config['on'].instanceReady = (evt: any) => {
	...
	private subscribe( editor: any ): void {
		...
		editor.on( 'paste', (evt: any) => {
			this.ngZone.run( () => {
				this.paste.emit( evt );
			} );
		} );

		editor.on( 'afterPaste', (evt: any) => {
			this.ngZone.run( () => {
				this.afterPaste.emit( evt );
			} );
		} );

		editor.on( 'dragend', (evt: any) => {
			this.ngZone.run( () => {
				this.dragEnd.emit( evt );
			} );
		});

		editor.on( 'dragstart', (evt: any) => {
			this.ngZone.run( () => {
				this.dragStart.emit( evt );
			} );
		} );

		editor.on( 'drop', (evt: any) => {
			this.ngZone.run( () => {
				this.drop.emit( evt );
			} );
		} );

		editor.on( 'fileUploadRequest', (evt: any) => {
			this.ngZone.run( () => {
				this.fileUploadRequest.emit(evt);
			} );
		} );

		editor.on( 'fileUploadResponse', (evt: any) => {
			this.ngZone.run( () => {
